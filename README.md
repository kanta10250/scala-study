...
- [02-calculating-factorials-in-scala](./02-calculating-factorials-in-scala/README.md)
  Scala のプログラムをファイルから実行する方法、作成したアプリケーションの読み方、sbt の常駐実行をまとめた資料

## 最後

まずは `01-scala-introduction` で基礎を確認し、その後に `02-calculating-factorials-in-scala` でファイルを使った Scala プログラムの実行、読解、`sbt` の基本的な使い方に進む構成になっている。
